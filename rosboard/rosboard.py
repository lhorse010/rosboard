...
from rosboard.subscribers.ros2trace_subscriber import Ros2traceSubscriber
...
class ROSBoardNode(object):
    ...
    def sync_subs(self):
        ...
        try:
            ...
            for topic_name in self.remote_subs:
                ...
                if topic_name == "ros2trace":
                    if topic_name not in self.local_subs:
                        rospy.loginfo("Subscribing to ros2trace [non-ros]")
                        self.local_subs[topic_name] = Ros2traceSubscriber(self.on_ros2trace)
                    continue
    ...
    def on_ros2trace(self, text):
        """
        ros2trace received. make it look like a rcl_interfaces/msg/Log and send it off
        """
        if self.event_loop is None:
            return
        ...
        self.event_loop.add_callback(
            ...
            [
                ROSBoardSocketHandler.MSG_MSG,
                {
                    "_topic_name": "ros2trace", # special non-ros topics start with _
                    "_topic_type": "rcl_interfaces/msg/Log",
                    "msg": text,
                },
            ]
